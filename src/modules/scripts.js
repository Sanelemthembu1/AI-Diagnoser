// Define the symptom keywords array
const key_words = [
...
// Initialize the user input JSON object with default values
let userInputJson = key_words.reduce((acc, keyword) => {
    acc[keyword] = "0";
    return acc;
}, {});

// Function to update the JSON object with user input symptoms
function updateSymptoms(userInput) {
    const symptoms = userInput.toLowerCase().split(',').map(s => s.trim());
    symptoms.forEach(symptom => {
        if (key_words.includes(symptom)) {
...
function sendDataToServer(data) {
    fetch('https://us-central1-aiplatform.googleapis.com/v1/projects/54118062613/locations/us-central1/endpoints/6703095672922439680:predict', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            //'Authorization' : 'AIzaSyB75p-w_RPH1XEJUWp-bEIjugZlImVIiqE' //here is the problem
        },
    ...
    .then(data => {
        ...
        addAgentMessage(`AI Diagnosis: ${JSON.stringify(data)}`);
    })
    .catch((error) => {
...
function handleUserInput() {
    ...
    updateSymptoms(userInput);
    sendDataToServer(userInputJson);
...
function addUserMessage(message) {
    ...
    userMessageDiv.classList.add('right-part');
    userMessageDiv.innerHTML = `<div class="user-chart"><p>${message}</p></div>`;
    chatSection.appendChild(userMessageDiv);
}
...
function addAgentMessage(message) {
    ...
    agentMessageDiv.classList.add('left-part');
    agentMessageDiv.innerHTML = `<div class="agent-chart"><p>${message}</p></div>`;
    chatSection.appendChild(agentMessageDiv);
}
...
document.getElementById('send').addEventListener('click', handleUserInput);
